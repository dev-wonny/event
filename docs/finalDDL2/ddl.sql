...
COMMENT ON COLUMN event_platform.event.supplier_id               IS '공급사 식별자 (supplier Table참조)';
...
COMMENT ON TABLE  event_platform.event_applicant           IS '참여자 기준';
COMMENT ON COLUMN event_platform.event_applicant.event_id  IS '이벤트 식별자 (조회용 값참조)';
COMMENT ON COLUMN event_platform.event_applicant.round_id  IS '회차 식별자';
...
CREATE UNIQUE INDEX uq_event_applicant_round_member_id
    ON event_platform.event_applicant (round_id, member_id)
    WHERE is_deleted = FALSE;
