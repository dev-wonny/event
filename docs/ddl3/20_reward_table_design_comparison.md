...
### 이유
- C의 구조는 "슬롯 조회 후 extension JOIN"이 항상 붙어야 해서 애플리케이션 코드 복잡도가 올라갑니다. 
- 타입이 3개일 때 이 비용을 치를 필요가 없어요. 
- 타입이 4~5개 넘어가면서 어드민 공통화 요구가 생기는 시점에 C로 마이그레이션하는 게 가장 현실적인 로드맵입니다.